...
import { calculateClusterHealth } from "../utils/clusterHealth";
...
function Dashboard() {
  ...
  const nodeMetrics = realtimeNodes.length > 0 ? realtimeNodes : initialNodeMetrics;
  const events = useClusterEvents();
  const recentEvents = events.slice(0, 5);
  const totalCPU = nodeMetrics.reduce((sum, node) => sum + node.cpuCores, 0);
  const totalMemory = nodeMetrics.reduce((sum, node) => sum + node.memoryMB, 0);
  const alerts = generateAlerts(nodeMetrics, podMetrics, deployments || []);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1md:grid-cols-4 gap-6 mb-10 ">
        <SummaryCard title="Nodes" value={nodeMetrics.length} />
        <SummaryCard title="Pods" value={podMetrics.length} />
        <SummaryCard title="CPU Usage" value={`${totalCPU.toFixed(2)} cores`} />
        <SummaryCard title="Memory Usage" value={`${totalMemory.toFixed(0)} MB`} />
      </div>
